...
// Financeiro não tem clientes_financeiro — qualquer cliente cadastrado é
...
// tarefaTipoId sempre revalidado contra o setor 'financeiro' — quem tem
// acesso concedido só a Configurações → Financeiro (não admin geral) não
// pode usar esta action pra mexer em vínculos de tarefa_tipos de outro
// setor, mesmo sabendo o id.
async function exigirTarefaTipoFinanceiro(supabase: NonNullable<Awaited<ReturnType<typeof exigirAdmin>>['supabase']>, tarefaTipoId: string): Promise<string | null> {
  const { data: tarefaTipo } = await supabase.from('tarefa_tipos').select('setor').eq('id', tarefaTipoId).single()
  if (!tarefaTipo || tarefaTipo.setor !== 'financeiro') return 'Tipo de tarefa não encontrado.'
  return null
}
...
export async function listarClienteIdsVinculados(
  ...
  const erroSetor = await exigirTarefaTipoFinanceiro(supabase, tarefaTipoId)
