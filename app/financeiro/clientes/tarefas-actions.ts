...
import { getAuthenticatedAdmin, podeEditarTarefaTipoFinanceiro } from '@/lib/supabase/server'
...
export interface TarefaFinanceiroAplicavel {
  tarefaTipoId: string
  nome: string
...
// tarefa_tipo_vinculos) com o catálogo de tarefa_tipos do Financeiro,
...
// mês/ano, se existir.
export async function listarTarefasFinanceiroDoCliente(
  clienteId: string,
  mes: number,
  ano: number,
): Promise<{ data: TarefaFinanceiroAplicavel[]; error: string | null }> {
  ...
  const { data: vinculosRaw, error: vinculosError } = await supabase
    ...
    .eq('tarefa_tipos.setor', 'financeiro')
  ...
  const { data: tarefasExistentes, error: tarefasError } = await supabase
    .from('tarefas').select('*')
    .eq('cliente_id', clienteId).eq('mes', mes).eq('ano', ano).eq('setor', 'financeiro')
    .in('tipo', nomes)
  ...
  const data: TarefaFinanceiroAplicavel[] = aplicaveis.map(v => ({
...
async function buscarOuCriarTarefa(
  ...
  const { data: existente } = await supabase
    ...
    .eq('cliente_id', clienteId).eq('mes', mes).eq('ano', ano).eq('tipo', tipo).eq('setor', 'financeiro')
    .maybeSingle()
  ...
  const { data: nova, error } = await supabase
    .from('tarefas')
    .insert({ cliente_id: clienteId, usuario_id: user.id, mes, ano, tipo, setor: 'financeiro', concluida: false })
    .select('id')
    .single()
...
function revalidarFicha(clienteId: string) {
  revalidatePath(`/financeiro/clientes/${clienteId}`)
}

export async function toggleTarefaFinanceiro(
  clienteId: string, tipo: string, mes: number, ano: number, concluida: boolean, data?: string,
): Promise<{ error: string | null }> {
  if (!(await podeEditarTarefaTipoFinanceiro(tipo))) return { error: 'Sem permissão para editar essa tarefa.' }
  const { supabase } = await getAuthenticatedAdmin()
...
export async function atualizarEtapaFinanceiro(
  ...
  const { data: tipoRow } = await supabase
    ...
    .eq('setor', 'financeiro').eq('nome', tipo)
    .maybeSingle()
...
export async function salvarRespostaTextoFinanceiro(
