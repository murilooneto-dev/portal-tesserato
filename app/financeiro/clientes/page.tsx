...
export const metadata = { title: 'Clientes — Tesserato Financeiro' }

export default async function ClientesFinanceiroPage() {
  ...
  // regra de app/financeiro/clientes/tarefas-actions.ts:22-45
  // (listarTarefasFinanceiroDoCliente), só que em lote pra todos os
  ...
  const [{ data: vinculosRaw }, { data: tarefaTiposRaw }, tarefas] = await Promise.all([
    ...
    buscarTodasTarefasDoMes<Pick<Tarefa, 'cliente_id' | 'concluida' | 'tipo'>>(
      supabase, mes, ano, 'cliente_id, concluida, tipo', 'financeiro',
    ),
  ])
