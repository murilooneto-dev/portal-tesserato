...
export default function DevLock({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="bg-[var(--fg)]/3 border border-[var(--fg)]/8 rounded-2xl p-6 flex flex-col items-center text-center gap-3">
      <p className="text-[var(--fg)]/60 text-sm font-semibold">🔒 Área restrita</p>
      <p className="text-[var(--fg)]/30 text-xs max-w-sm">
      ...
      <div className="flex flex-col gap-2 w-full max-w-xs mt-2">
        <input
          ...
          className="px-3 py-2 rounded-lg bg-[var(--fg)]/5 border border-[var(--fg)]/10 text-[var(--fg)] text-sm focus:outline-none focus:border-[var(--accent)]/50"
        />
        <input
          type="password"
          placeholder="Senha"
