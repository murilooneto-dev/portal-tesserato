...
app/(comum)/vinculos/actions.ts                   + remediação: criarVinculos/excluirVinculo (Server Actions guardadas, substituem escrita direta do browser)
...
| `criarVinculos({ setorOrigem, setorDestino, pares })` / `excluirVinculo(id)` | `app/(comum)/vinculos/actions.ts` (novo) | Exige `role='admin'` **e** sessão `ts_admin` válida antes de escrever em `tarefa_vinculos`. `pares` é uma lista de `{ tipoOrigem, tipoDestino }` (bulk insert, com dedup server-side por par). Substitui a escrita direta do browser que existia em `VinculosClient.tsx`. |
